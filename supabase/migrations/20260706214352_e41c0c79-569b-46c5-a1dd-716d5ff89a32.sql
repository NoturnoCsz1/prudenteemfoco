...
BEGIN
  ...
  IF _tok.status <> 'active' THEN
    -- Differentiate "already used / capacity reached" from a genuinely expired/revoked token.
    IF _tok.status = 'expired'
       AND _tok.capacity_limit IS NOT NULL
       AND _tok.usage_count >= _tok.capacity_limit THEN
      _blocked_reason := 'capacity_reached';
    ELSE
      _blocked_reason := 'token_' || _tok.status::text;
    END IF;
    ...
    PERFORM public.record_audit_event(
      ...
      jsonb_build_object('token_id', _tok.id, 'reason', _blocked_reason),
      NULL, NULL
    );
    RETURN QUERY SELECT _session_id, NULL::uuid, 'blocked'::public.access_session_status,
                        _blocked_reason, NULL::text, 0;
    RETURN;
  END IF;
