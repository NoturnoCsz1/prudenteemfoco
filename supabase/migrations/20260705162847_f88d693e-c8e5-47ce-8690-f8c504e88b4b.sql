...
-- RPC pública para tracking de eventos site-wide (sem event_id)
...
DECLARE
  _id uuid;
BEGIN
  -- Whitelist: apenas kinds site-wide são aceitos por esta função
  IF _kind NOT IN ('home_page_view', 'eventos_list_view') THEN
    RETURN NULL;
  END IF;

  INSERT INTO public.hotsite_click_events (
    organization_id, event_id, kind,
    utm_source, utm_medium, utm_campaign, utm_content, utm_term, referrer
  ) VALUES (
    NULL, NULL, _kind,
    NULLIF(_utm_source, ''), NULLIF(_utm_medium, ''), NULLIF(_utm_campaign, ''),
    NULLIF(_utm_content, ''), NULLIF(_utm_term, ''), NULLIF(_referrer, '')
  )
...
REVOKE ALL ON FUNCTION public.track_site_event(public.hotsite_click_kind, text, text, text, text, text, text) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.track_site_event(public.hotsite_click_kind, text, text, text, text, text, text) TO anon, authenticated;

-- Atualiza analytics para incluir home_page_view e eventos_list_view
...
BEGIN
  ...
  WITH scoped AS (
    ...
    WHERE h.created_at >= _from
      ...
      AND (
        (h.organization_id IS NOT NULL AND public.is_active_org_member(_uid, h.organization_id))
        OR (h.organization_id IS NULL AND _is_member)
      )
  )
  SELECT jsonb_build_object(
    'site_visits', COALESCE(SUM(CASE WHEN kind = 'home_page_view' THEN 1 ELSE 0 END), 0),
    'eventos_list_views', COALESCE(SUM(CASE WHEN kind = 'eventos_list_view' THEN 1 ELSE 0 END), 0),
