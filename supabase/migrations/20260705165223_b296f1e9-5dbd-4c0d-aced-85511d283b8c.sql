CREATE OR REPLACE FUNCTION public.admin_site_analytics(_from timestamp with time zone, _to timestamp with time zone)
 RETURNS jsonb
 LANGUAGE plpgsql
...
BEGIN
  ...
  RETURN jsonb_build_object('totals', COALESCE(_totals, '{}'::jsonb), 'top_events', COALESCE(_top, '[]'::jsonb));
END;
$function$;